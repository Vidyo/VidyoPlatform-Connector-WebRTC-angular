...
export class VidyoConnector extends Chat {
    ...
    getParticipantName(participant: any, cb: any): void {
        ...
        participant.GetName().then(function (name: any) {
            cb(name);
        }).catch(function () {
            cb("GetNameFailed");
        });
    ...
    parseUrlParameters() {
        ...
        this.configParams.disableAudioLevelMonitor = this.getUrlParameterByName("disableAudioLevelMonitor");
        this.configParams.dynamicAudioSources = this.getUrlParameterByName("dynamicAudioSources");
        this.configParams.enableAudioOnlyMode = this.getUrlParameterByName("enableAudioOnlyMode");
        this.configParams.enableLoki = this.getUrlParameterByName("enableLoki") === 'true';
        ...
        let showAdvancedSettings = this.getUrlParameterByName("showAdvancedSettings");
        const enableAutoReconnect = this.getUrlParameterByName("enableAutoReconnect");
        const maxReconnectAttempts = this.getUrlParameterByName("maxReconnectAttempts");
        const reconnectBackoff = this.getUrlParameterByName("reconnectBackoff");

        if (enableAutoReconnect) {
            this.configParams.enableAutoReconnect = ['true', '1'].includes(enableAutoReconnect);
        }
    ...
    registerDeviceListeners() {
        ...
        vidyoConnector.RegisterLocalCameraEventListener({
            ...
            onAdded: function (localCamera: any) {
                thisClass.videoDevices.push({ value: window.btoa(localCamera.id), label: localCamera.name });
                ...
                if (localCamera) {
                    ...
                    thisClass.videoDevices.forEach(item => {
                        ...
                        if (item.value != window.btoa(localCamera.id)) {
                            ...
                            tmp.push({ value: window.btoa(localCamera.id), label: localCamera.name, selected: true });
                            notFound = false;
                        }
                    });
        ...
        vidyoConnector.RegisterLocalMicrophoneEventListener({
            ...
            onAdded: function (localMicrophone: any) {
                thisClass.micDevices.push({ value: window.btoa(localMicrophone.id), label: localMicrophone.name });
            ...
            onSelected: function (localMicrophone: any) {
                if (thisClass.localParticipant) {
                    thisClass.setLocalParticipantMicrophonePrivacy(thisClass.microphonePrivacy);
                }
                if (localMicrophone) {
                    ...
                    thisClass.micDevices.forEach(item => {
                        ...
                        if (item.value != window.btoa(localMicrophone.id)) {
                            ...
                            tmp.push({ value: window.btoa(localMicrophone.id), label: localMicrophone.name, selected: true });
                            notFound = false;
                        }
                    });
            ...
            onStateUpdated: function (localMicrophone: any, state: any) {
                ...
                if (thisClass.localParticipant) {
                    this.setLocalParticipantMicrophonePrivacy(thisClass.microphonePrivacy);
                }
            }
        }).then(function () {
        ...
        vidyoConnector.RegisterLocalSpeakerEventListener({
            ...
            onAdded: function (localSpeaker: any) {
                thisClass.speakerDevices.push({ value: window.btoa(localSpeaker.id), label: localSpeaker.name });
                ...
                if (localSpeaker) {
                    ...
                    thisClass.speakerDevices.forEach(item => {
                        ...
                        if (item.value != window.btoa(localSpeaker.id)) {
                            ...
                            tmp.push({ value: window.btoa(localSpeaker.id), label: localSpeaker.name, selected: true });
                            notFound = false;
                        }
                    });
        ...
        vidyoConnector.RegisterRemoteMicrophoneEventListener({
             ...
             * @param state String
             */
            onStateUpdated: function (remoteMicrophone: any, participant: any, state: any) {
                // mic on
    ...
    handleReconnect() {
        ...
        this.vidyoConnector.RegisterReconnectEventListener({
            onReconnecting: (attempt: any, attemptTimeout: any, lastReason: any) => {
     ...
     */
    handleSharing() {
        let isSafari = navigator.userAgent.indexOf('Chrome') < 0 && navigator.userAgent.indexOf('Safari') >= 0;
        ...
        function StartMonitorShare() {
            ...
            thisClass.vidyoConnector.RegisterLocalMonitorEventListener({
                /**
                ...
                onAdded: function (localMonitorShare: any) {
                    if (localMonitorShare.name != "") {
                        thisClass.monitorDevices.push({ value: window.btoa(localMonitorShare.id), label: localMonitorShare.name });
                        thisClass.monitorShares[window.btoa(localMonitorShare.id)] = localMonitorShare;
                    ...
                    if (localMonitorShare) {
                        ...
                        thisClass.monitorDevices.forEach(item => {
                            ...
                            if (item.value != window.btoa(localMonitorShare.id)) {
                                ...
                                tmp.push({ value: window.btoa(localMonitorShare.id), label: localMonitorShare.name, selected: true });
                                notFound = false;
                            }
                        });
    ...
    connectToConference() {
        ...
        */
        ...
        this.vidyoConnector.ConnectToRoomAsGuest({
            ...
            onDisconnected: function (reason: any) {
                ...
                scope.connectorDisconnected("Disconnected", "Call Disconnected: " + reason);
                scope.statsSendingTool.stopSendingStats();
            }
