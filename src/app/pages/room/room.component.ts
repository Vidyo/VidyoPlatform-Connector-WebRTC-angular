...
export class RoomComponent extends VidyoConnector implements OnInit {
  ...
  initVidyoConnector(): void {
    ...
    this.getVC().CreateVidyoConnector({
      ...
      // show vc version
      vc.GetVersion().then((version: string) => scope.vcVersion = version, () => scope.vcVersion = "GetVersion failed");
      ...
      scope.statsSendingTool = new LokiImplementation().getLokiImplementation(scope.configParams.enableLoki, scope.configParams.lokiBaseUrl, scope.vidyoConnector);
      scope.handleParticipantChange();
      scope.handleReconnect();
      scope.handleSharing();
      scope.handleChat();

      if (scope.configParams && scope.configParams.disableAudioLevelMonitor === 'true') {
        scope.vidyoConnector.SetAdvancedConfiguration({ disableAudioEnergyMonitor: true })
      }

      if (scope.configParams && scope.configParams.dynamicAudioSources) {
        scope.vidyoConnector.SetAdvancedConfiguration({ dynamicAudioSources: scope.configParams.dynamicAudioSources })
      }

      // If enableDebug is configured then enable debugging
      if (scope.configParams && scope.configParams.enableDebug === "1") {
        scope.vidyoConnector.EnableDebug({ port: 7776, logFilter: "debug sent@VidyoClient enter@VidyoClient all@VidyoSignaling" }).then(function () {
      ...
      if (scope.configParams && scope.configParams.enableAutoReconnect) {
        scope.vidyoConnector.SetAdvancedConfiguration({ enableAutoReconnect: scope.configParams.enableAutoReconnect });
      }

      if (scope.configParams && scope.configParams.maxReconnectAttempts) {
        scope.vidyoConnector.SetAdvancedConfiguration({ maxReconnectAttempts: scope.configParams.maxReconnectAttempts });
      }

      if (scope.configParams && scope.configParams.reconnectBackoff) {
        scope.vidyoConnector.SetAdvancedConfiguration({ reconnectBackoff: scope.configParams.reconnectBackoff });
      }

      if (scope.configParams && scope.configParams.enableAudioOnlyMode === 'false') {
        scope.vidyoConnector.SetAdvancedConfiguration({ enableAudioOnlyMode: false });
      }
